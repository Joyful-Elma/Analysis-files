...
void ShowSPDConfiguration(Int_t runNb=244340, const char *ocdblocation="alien://Folder=/alice/data/2015/OCDB/",bool threed=kFALSE){
  TString ocdb(ocdblocation);
  Bool_t grid =kFALSE;
  if(ocdb.Contains("alien")) grid=kTRUE;
  gStyle->SetOptStat(0);
  ...
  h->ReadDeadFromDB(runNb,"alien://Folder=/alice/data/2015/OCDB/");
  ...
  for(Int_t i=0; i<80; i++){
    if((h->GetNrBad(i))<1) {
      ...
      if(i>75){
      if(phiDown<0.1) phiDown+=2*TMath::Pi();
      }
/*
      Double_t mod = TMath::Sqrt(global1[0]*global1[0] + global1[1]*global1[1] + global1[2]*global1[2]);
      Double_t theta = TMath::ACos(global1[2]/mod);
...
*/
  ...
  for(Int_t i=80; i<240; i++){
    ...
    if((h->GetNrBad(i))<1) {
      ...
      Double_t mod = TMath::Sqrt(global[1][0]*global[1][0] + global[1][1]*global[1][1] + global[1][2]*global[1][2]);
      Double_t theta = TMath::ACos(global[1][2]/mod);
      printf("module %i - eta : %f \n",i,TMath::Log(TMath::Tan(0.5*theta)));
   }
  }
